        ...
        <form method="POST" action="{{ route('partidos.update', $partido->idpartido) }}">
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" name="parfec" class="form-control"
                       value="{{ old('parfec', \Carbon\Carbon::parse($partido->parfec)->format('Y-m-d\TH:i')) }}"
                       required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="idclub_local" class="form-control" required>
                    @foreach($clubes as $club)
                        <option
                            value="{{ $club->idclub }}" {{ $club->idclub == $partido->idclub_local ? 'selected' : '' }}>
                            {{ $club->clubnom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="idclub_visitante" class="form-control" required>
                    @foreach($clubes as $club)
                        <option
                            value="{{ $club->idclub }}" {{ $club->idclub == $partido->idclub_visitante ? 'selected' : '' }}>
                            {{ $club->clubnom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="pargolloc" class="form-control"
                       value="{{ old('pargolloc', $partido->pargolloc) }}" required>
            </div>

            <div class="mb-3">
                <label>Fecha</label>
                <input type="number" name="parfechas" class="form-control"
                       value="{{ old('parfechas', $partido->parfechas) }}" required>
            </div>

            <div class="mb-3">
                <label>Goles Visitante</label>
                <input type="number" name="pargolvis" class="form-control"
                       value="{{ old('pargolvis', $partido->pargolvis) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="idcampeonato" class="form-control" required>
                    @foreach($campeonatos as $camp)
                        <option
                            value="{{ $camp->idcampeonato }}" {{ $camp->idcampeonato == $partido->idcampeonato ? 'selected' : '' }}>
                            {{ $camp->campnom }}
                        </option>
                    @endforeach
                </select>
